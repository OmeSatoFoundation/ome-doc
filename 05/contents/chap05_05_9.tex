...
\begin{table}[H]
	...
	\begin{equation}
	 ...
	 \label{eq:distance}
	\end{equation}
三角測量の原理により、ある三角形があったとき一辺の長さとそれに対する角の角度がわかれば三角形が決定します（\ruby{余弦}{よげ|ん}定理）。距離センサーは\ruby{光源}{こう|げん}と受光器で成っています。受光器は光源から出た光が対象にぶつかって\ruby{反射}{はん|しゃ}してきた光の角度を計測することが出来ます。光源と受光器は固定されているので、その間の距離と対する角の角度がわかり、これと三角関数を使うと対象との距離が求まります\\ \hline
  \end{tabular}
\end{table}
