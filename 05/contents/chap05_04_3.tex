...
\item リモコンの信号を受け取り、ファイルに記録します。
 \begin{enumerate}[1]
  ...
  \item \ruby{違}{ちが}う信号を受け取らないように、赤外線受信を止めます。Worningが表示される場合がありますが、気にしなくても良いです。 \\ \code{sudo service lircd stop}
...
\item ファイルに記録された信号を、赤外線LEDから送信する。
 \begin{enumerate}[1]
  ...
  \item 赤外線を送ります。fanは設定ファイルに書いたリモコンの名前、onoffは設定ファイルに書いた信号の名前に変えましょう。\\ \code{irsend SEND\_ONCE \textcolor{red}{fan onoff}} \\ この際、センサーボードの赤外線LEDは家電から10cmくらい離し、間をさえぎるものが無いようにしましょう。
 \end{enumerate}
\end{enumerate}
