...
%注マージ後ページ参照を解決
22ページでは、echoコマンドはruby{引数}{ひき|すう}で与えられた\ruby{文字列}{も|じ|れつ}をそのまま\ruby{標準}{ひょう|じゅん}出力に出力するコマンドとして\ruby{登場}{とう|じょう}しました。
今回は他のコマンドの実行結果をechoコマンドの引数に与えて、それを標準出力に出力します。
コマンドの中で使用される \$() はコマンド\ruby{置換}{ち|かん}を行うための構文です。コマンド\ruby{置換}{ち|かん}を使用すると、中のコマンドの\{実行結果}{じっ|こう|けっ|か}を、別のコマンドの\ruby{引数}{ひき|すう}として使用することができます。
