...
\begin{tcolorbox}[title=\useOmetoi]
    \begin{enumerate}
        \addex{03ディレクトリの中身を出力しましょう。}
        \addex{03ディレクトリの中のファイルの大きさを確認してみましょう。}
        \addex{03/rensyu/kokugo/syosetsu.txtの行数を数えてみましょう。}
        \addex{"I love programming!"という文字列を出力してみましょう。}
    \end{enumerate}
\end{tcolorbox}
...
catは、入力を何もせずに出力するコマンドです。

\begin{lstlisting}[caption=catコマンドの実行例, label=cat_example]
<#green#pi@raspberrypi#>:<#blue#~ $#> cat 
Hello World!        # キーボードからの入力
Hello World!        # 出力
I love programming! # キーボードからの入力
I love programming! # 出力
\end{lstlisting}

この例では、catコマンドを使って、キーボードから入力した文字列をそのまま出力しています。
catコマンドを実行後、キーボードで文字を入力し、エンターキーを押すと、入力した文字列がそのまま出力されます。
終了したい場合は、Ctrl+Dを押します。

また、catはファイルの中身を表示するためにも使われます。\\
\colorbox[gray]{0.8}{cat \textvisiblespace 表示したいファイルのパス} を入力することで、ファイルの中身を表示することができます。

\begin{lstlisting}[caption=catコマンドでファイルの中身を表示する例, label=cat_file_example]
<#green#pi@raspberrypi#>:<#blue#~ $#> cat ~/03/rensyu/sansu/sansu.txt
1 + 1 = 2<#green#pi@raspberrypi#>:<#blue#~ $#> 
\end{lstlisting}

この例では、\textasciitilde/03/rensyu/sansu/sansu.txtの中身を表示しています。

\begin{tcolorbox}[title=\useOmetoi]
    \begin{enumerate}
        \addex{\textasciitilde/03/rensyu/kokugo/syosetsu.txtの中身を表示してみましょう。}
    \end{enumerate}
\end{tcolorbox}

\subsubsection{tacコマンド}
tacコマンドは、行を逆順に出力するコマンドです。\\
\colorbox[gray]{0.8}{tac \textvisiblespace 表示したいファイルのパス} を入力することで、ファイルの中身を逆順に表示することができます。
...
\colorbox[gray]{0.8}{shuf \textvisiblespace 表示したいファイルのパス} を入力することで、ファイルの中身の行をランダムに入れ替えて表示することができます。
...
\colorbox[gray]{0.8}{head \textvisiblespace 表示したいファイルのパス \textvisiblespace -n \textvisiblespace 行数} を入力することで、ファイルの中身の先頭のいくつかの行を表示することができます。
...
\colorbox[gray]{0.8}{tail \textvisiblespace 表示したいファイルのパス \textvisiblespace -n \textvisiblespace 行数} を入力することで、ファイルの中身の末尾のいくつかの行を表示することができます。
...
\colorbox[gray]{0.8}{sort \textvisiblespace 表示したいファイルのパス} を入力することで、ファイルの中身の行を順番に並べ替えて表示することができます。
...
また、アルファベットの大文字と小文字同士を区別せずに並べ替えをします。

\subsubsection{grepコマンド}
grepコマンドは、検索パターンに一致する行を出力するコマンドです。\\
\colorbox[gray]{0.8}{grep \textvisiblespace 検索パターン \textvisiblespace 表示したいファイルのパス} を入力することで、ファイルの中身から検索パターンに一致する行を表示することができます。
