...
エイリアスは alias コマンドで追加できます。
\begin{description}
\item[\texttt{alias}\textvisiblespace 名前 = 'コマンド']
コマンドの別名として名前を設定します。
\end{description}
...
\end{description}
に l という別名を付けたいとします。
つまり、lt と実行したら ls -alF が実行されるようにしたいということです。
...
しかし残念ながら、bash を閉じると今設定した別名は消えてしまいます。

では、別名を保存しておくにはどうすればいいのでしょうか。bash 起動時に毎回手動で alias コマンドを実行すればいいのでしょうか。
実は、bash は起動時に ~/.bash{\_}aliases というファイルの中に書いてあるコマンドを実行してくれます。

ホームディレクトリに .bash{\_}aliases というファイルを作成し編集します。

.bash{\_}aliases には、次のようにコマンドを記述し、保存します。
\begin{lstlisting}[caption=.bashaliasesの書き方1, label=bashAliasesGrammar1]
alias 名前='コマンド'
alias name='command'
            :
            :
...
% \begin{lstlisting}[caption=.bashaliasesの書き方2, label=bashAliasesGrammar2]
%     alias 設定したい別名=設定したいコマンド 設定したい別名=設定したいコマンド 設定したい別名=設定したいコマンド ....
% \end{lstlisting}

% 後者のように記述できるため、設定したいコマンドにスペースが含まれていた場合、bash はスペースの後の文字列を新たな alias として認識し、意図とは異なった alias を設定してしまいます。

% 設定したいコマンドにスペースを含ませる場合、次のように設定したいコマンドをシングルクオートで囲みます。

保存が終わったら、.bash{\_}aliases の中のコマンドを実行しすることで、別名を適用します。

% ラズベリーパイで bash が起動したとき、.bashrc というファイルが自動で読み込まれます。さらにその中で .bash{\_}aliases を読み込む記述があります。ただし、そのタイミングでしか読み込まれないので、ファイルを編集した後に読み込みなおす必要があります。
% また、bash{\_}aliases に上で述べたように正しく記述しないと、読み込みの際に予期せぬエラーが発生して bash が正常に動作しなくなる可能性があります。.bash{\_}aliases を編集するときは注意して行ってください。

source というコマンドで .bash{\_}aliases の中のコマンドを実行します。
\begin{lstlisting}[caption=.bashaliasesの読込, label=sourceBashAliases]
<#green#pi@raspberrypi#>:<#blue#~ $#> source ~/.bash_aliases
<#green#pi@raspberrypi#>:<#blue#~ $#>
\end{lstlisting}

上のように実行して何も表示されなければ正常に適用できています。

それでは、次の問題に取り組んでみましょう。

\begin{enumerate}
    \addex{
        rm -i というコマンドに rm と別名を付けてみましょう。
    }
    \addex{
