...
\subsection{計算をする}
コマンドの説明： echo\textvisiblespace\$(コマンド) \\
22ページでは、echoコマンドは引数で与えられた文字列をそのまま標準出力に出力するコマンドとして登場しました。
今回はechoコマンドから他のコマンドを実行します。
コマンドの中で使用される \$() はコマンド置換を行うための構文です。コマンド置換
を使用すると、別のコマンドの出力を取得し、その出力を別のコマンドの引数として使用することができます。
\begin{lstlisting}[caption=echo コマンドを使った例1, label=echo]
<#green#pi@raspberrypi#>:<#blue#~/ $#> echo $(ls)
\end{lstlisting} 

\begin{lstlisting}[caption=lsコマンドと比較, label=ls]
<#green#pi@raspberrypi#>:<#blue#~/ $#> ls
\end{lstlisting}

二つのコマンドから同じ結果を得られることがわかります。\$()の中に書いたコマンドが実行されます。
echoコマンドの他の使い方を見てみましょう。echo \$((式))と打つと計算結果を出力してくれます。
ここで内側にも()があることに注意しましょう。これはシェルが内側の算術式の評価結果を変数として認識し、その結果を表示するために使用します。
つまり、数字の計算をするためのものです。

\newpage
\begin{lstlisting}[caption=echo コマンドを使った例2, label=echo]
...
このように電卓を使って計算したいものを目の前のコンピュータに計算してもらえたら便利ですね
\vskip\baselineskip
\noindent ターミナルでの四則演算\\
足し算 $+$\\
引き算 $-$\\
